...
import { PRODUCTS as CATALOG_PRODUCTS, type CatalogProduct } from '../data/catalog'
...
export const PRODUCTS: Product[] = [
  { id: 'cargador-baterias',      qty: 1,  name: 'Cargador Amarillo de Baterías', photos: CHARGER_PHOTOS,
    ...
    searchTags: ['bateria', 'carga', 'auto', 'moto', 'arrancar', 'voltaje', 'battery', 'charger', 'car'], priceMXN: 349 },
  { id: 'arrancador-vehiculos',   qty: 1,  name: 'Cargador de Baterías Inteligente para Autos y Motos, con Pantalla Digital', photos: VEHICLE_CHARGER_PHOTOS,
    ...
    searchTags: ['bateria', 'carga', 'auto', 'moto', 'arrancar', 'inteligente', 'battery', 'charger', 'smart'], priceMXN: 599 },
  ...
  { id: 'lampara-recargable-powerbank', qty: 1, name: 'Lámpara LED Recargable Multifunción con Power Bank (Foco, Luz COB y USB Tipo-C)', photos: RECHARGEABLE_WORKLIGHT_PHOTOS,
    ...
    searchTags: ['linterna', 'lampara', 'luz', 'led', 'recargable', 'powerbank', 'foco', 'flashlight', 'lamp'], priceMXN: 799 },
  { id: 'camara-bullet-dome',     qty: 1,  name: 'Cámara de Vigilancia Triple Lente (2 Bullet + 1 Domo), WiFi con Visión Nocturna Inteligente', photos: BULLET_DOME_CAMERA_PHOTOS,
    ...
    searchTags: ['camara', 'vigilancia', 'seguridad', 'cctv', 'domo', 'bullet', 'nocturna', 'wifi', 'camera', 'security', 'surveillance', 'night'], priceMXN: 1299 },
  { id: 'pest-repeller',          qty: 1,  name: 'Repelente Electrónico Ultrasónico de Plagas (Ratones, Insectos y Arañas)', photos: PEST_REPELLER_PHOTOS,
    ...
    searchTags: ['plaga', 'raton', 'rata', 'insecto', 'cucaracha', 'hormiga', 'araña', 'ultrasónico', 'pest', 'mice', 'rat', 'bug', 'repeller'], priceMXN: 249 },
  { id: 'linterna-camping-lifegear', qty: 1, name: 'Linterna de Campamento LED Life Gear, con Luz de Emergencia Roja', photos: CAMPING_LANTERN_PHOTOS,
    ...
    searchTags: ['linterna', 'lampara', 'luz', 'camping', 'emergencia', 'led', 'lantern', 'torch', 'outdoor'], priceMXN: 499 },
  { id: 'reloj-led-anaranjado',   qty: 5,  name: 'Relojes de LEDs Anaranjados (1 Rosa) para Habitación, con Control de Apagado', photos: LED_CLOCK_PHOTOS,
    ...
    searchTags: ['reloj', 'despertador', 'alarma', 'hora', 'temperatura', 'led', 'habitacion', 'clock', 'alarm', 'bedroom'], priceMXN: 349 },
  { id: 'camara-ip',              qty: 10, name: 'Cámaras IP de Vigilancia', photos: IP_CAMERA_PHOTOS,
    ...
    searchTags: ['camara', 'vigilancia', 'seguridad', 'ip', 'wifi', 'mini', 'espia', 'nocturna', 'camera', 'security', 'surveillance', 'spy'], priceMXN: 449 },
  { id: 'baliza-giratoria',       qty: 1,  name: 'Baliza LED Giratoria Roja con Zumbador 12/24V', photos: BEACON_PHOTOS,
    ...
    searchTags: ['baliza', 'luz', 'giratoria', 'emergencia', 'zumbador', 'auto', 'beacon', 'light', 'rotating', 'emergency'], priceMXN: 1499 },
  { id: 'almohada-cuello',        qty: 1,  name: 'Almohada de Viaje para Cuello con Memory Foam y Gel', photos: NECK_PILLOW_PHOTOS,
    ...
    searchTags: ['almohada', 'cuello', 'viaje', 'descanso', 'avion', 'pillow', 'neck', 'travel', 'foam', 'gel'], priceMXN: 349 },
  { id: 'palancas-llantas',       qty: 2,  name: 'Llave de Matraca para Birlos, 13.4 Pulgadas con Reversa', photos: RATCHET_WRENCH_PHOTOS,
    ...
    searchTags: ['llave', 'matraca', 'llanta', 'neumatico', 'birlo', 'herramienta', 'auto', 'wrench', 'tool', 'tire'], priceMXN: 449 },
  { id: 'camara-wireless',        qty: 1,  name: 'Cámara de Vigilancia Triple Lente 5K, WiFi con App (Pan 355° / Tilt 90°)', photos: WIRELESS_CAMERA_PHOTOS,
    ...
    searchTags: ['camara', 'vigilancia', 'seguridad', 'wifi', '5k', 'nocturna', 'movimiento', 'camera', 'security', 'surveillance', 'wireless', 'motion'], priceMXN: 2499 },
  { id: 'camara-355',             qty: 1,  name: 'Cámara de Vigilancia Grande 355° de Cobertura', photos: BIG_CAMERA_360_PHOTOS,
    ...
    searchTags: ['camara', 'vigilancia', 'seguridad', '8k', 'zoom', 'wifi', 'grande', 'camera', 'security', 'surveillance', '360'], priceMXN: 1999 },
  { id: 'camara-micro',           qty: 1,  name: 'Micro Cámara Espía WiFi con Detección de Movimiento y Alertas a la App', photos: MICRO_CAMERA_PHOTOS,
    ...
    searchTags: ['camara', 'espia', 'spy', 'mini', 'micro', 'oculta', 'vigilancia', 'seguridad', 'movimiento', 'camera', 'security', 'hidden', 'motion'], priceMXN: 499 },
...
export const HOME_PRODUCTS_CATALOG: CatalogProduct[] = PRODUCTS
  .filter(p => p.priceMXN != null)
  .map(p => ({
    id: `home-${p.id}`,
    nameEs: p.name,
    nameEn: p.nameEn ?? p.name,
    descEs: p.features?.[0] ?? '',
    descEn: p.featuresEn?.[0] ?? p.features?.[0] ?? '',
    priceMXN: p.priceMXN!,
    rating: 4.7,
    reviews: 0,
    category: HOME_CAT_MAP[p.category] ?? 'home',
    images: p.photos ?? [],
  }))
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="mt-auto flex flex-col gap-1.5">
          <Link to={`/products/${p.id}`}
            ...
            style={{ color: '#0C0C0C', background: 'linear-gradient(135deg, #8B6000 0%, #C8900A 18%, #E8B820 35%, #FFF4C4 50%, #F0D040 65%, #C8A020 82%, #8B6000 100%)', boxShadow: '0 2px 8px rgba(212,175,55,0.35)' }}>
...
export default function ProductsPage() {
  ...
  const [navH, setNavH] = useState(() => window.innerWidth < 640 ? 104 : 64)

  useEffect(() => {
    const onResize = () => setNavH(window.innerWidth < 640 ? 104 : 64)
  ...
  const catalogMatches = (prod: typeof CATALOG_PRODUCTS[0]) => {
    if (!q) return false
    if (norm(prod.nameEs).includes(q) || norm(prod.nameEn).includes(q)) return true
    if (norm(prod.descEs).includes(q) || norm(prod.descEn).includes(q)) return true
    return false
  }
  ...
  const filteredCatalog = q ? CATALOG_PRODUCTS.filter(catalogMatches).slice(0, 12) : []
  ...
  return (
    <div style={{ background: '#F5F5F5', minHeight: '100vh' }}>
      ...
      <div className="fixed left-0 right-0 z-40 flex items-center px-3 sm:px-8 gap-2"
        style={{ top: navH, height: 52, background: '#FFFFFF', borderBottom: '1px solid #EBEBEB' }}>
      ...
      {searchFocused && !search.trim() && (
        ...
        <div className="fixed left-0 right-0 z-50 flex justify-center px-3 sm:px-6"
          style={{ top: navH + 52, pointerEvents: 'none' }}>
          <div ref={suggestionsRef} className="w-full max-w-4xl flex overflow-hidden rounded-2xl"
            onMouseDown={e => e.preventDefault()}
            style={{ pointerEvents: 'auto', maxHeight: `calc(100vh - ${navH + 50}px)`, background: '#FFFFFF', boxShadow: '0 12px 40px rgba(0,0,0,0.2)', border: '1px solid rgba(212,175,55,0.15)' }}>
      ...
      <div ref={catBarRef} className="fixed left-0 right-0 z-40 flex flex-wrap items-center gap-1.5 px-3 sm:px-8 py-2"
        style={{ top: navH + 52, background: '#FFFFFF', borderBottom: '1px solid rgba(212,175,55,0.18)' }}>
        <button type="button" onClick={() => setActiveCategory('all')}
          ...
          style={activeCategory === 'all'
            ? { background: 'linear-gradient(135deg, #8B6000 0%, #C8900A 18%, #E8B820 35%, #FFF4C4 50%, #F0D040 65%, #C8A020 82%, #8B6000 100%)', color: '#0C0C0C', border: '1.5px solid #C8900A' }
      ...
      <main style={{ paddingTop: `${navH + 52 + catBarH}px` }} className="pb-10">
<div className="px-2 sm:px-4 md:px-8">
        {filtered.length === 0 && filteredMayorista.length === 0 && filteredCatalog.length === 0 && q ? (
            ...
            {filteredCatalog.length > 0 && (
              <div className="max-w-7xl mx-auto mt-6">
                <p className="text-xs font-black uppercase tracking-widest mb-3 px-1" style={{ color: 'rgba(212,175,55,0.7)' }}>
                  {lang === 'en' ? 'Also in Products' : 'También en Productos'}
                </p>
                  ...
                  {filteredCatalog.map(item => (
                    <Link key={item.id} to="/productos"
                      ...
                      <div className="w-full aspect-[4/3] overflow-hidden flex items-center justify-center" style={{ background: '#F5F5F5' }}>
                        {item.images?.[0]
                          ? <img src={item.images[0]} alt={lang === 'en' ? item.nameEn : item.nameEs} className="w-full h-full object-contain" />
